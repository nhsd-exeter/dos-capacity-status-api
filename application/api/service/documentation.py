...
capacity_service_api_desc = """This is the Capacity Status API.
    ...
    of restful endpoints that can be called from third party systems.
    ...
    sent through as part of the request in the call to the API endpoints.
    ...
    the API's authorisation process.

    <B>API Authorisation</B>
...
response_entities_desc = {
    "rag_status": "<li><B>Capacity status</B> - the capacity status of the service that is updated to the capacity \
        status given in the request payload. The capacity status can be set to Red, Amber, or Green.</li>",
    "reset_status_in": "<li><B>Reset time</B> - the date and time when the service will automatically transition back \
        to a Green capacity status having been set to either Amber or Red by the request. The reset time will be set \
        to the date and time when the capacity status is updated (or maintained) plus the amount of time in minutes \
        as defined by the resetStatusIn value given in the request payload. If this value is not provided, the reset \
        time will default to 4 hours from the time the capacity status is updated. A capacity status change to Red or \
        Amber for a service can be set to persist for a maximum period of 120 hours per request before automatically \
        being reset back to Green. This means that if a service needs to be Red or Amber for over 24 hours, multiple \
        requests through this API for that service will be required. </li>",
    "notes": "<li><B>Notes</B> - a free text field providing the opportunity for any additional notes to be logged \
        regarding the capacity status change. Notes given in the request payload will be appended to the text: \
        Capacity status set by the Capacity Service API -. </li>",
    "last_updated": "<li><B>Last updated date/time</B> - a timestamp of when the service was updated by the API.</li>",
    "by": "<li><B>Modified by</B> - the name of the DoS User (associated with the API Token) who last updated the \
        capacity information of the service.</li>",
}
description_model = {
    "post__firstline": "This endpoint will update the capacity (RAG) status of a single service in DoS as specified in the \
        request body JSON payload. The format of the JSON payload is described further down in this document.",
    "business_logic_header": "</BR></BR> <B><U>Business Logic</B></U></BR></BR>",
    "request_validation_header": "</BR> </BR> <B><U>Request Validation</B></U></BR></BR>",
    "post__business_logic_content": "Upon processing a successful request, the API will return a success response containing \
        the capacity status information for the given service and will update DoS with the following information: </BR> <ul>"
    ...
    + "</ul></BR> Requests that fail the request validation rules will not be processed and will return a validation error \
        response (see below for the validation rules). </BR></BR> Requests that fail to be processed due to an error or failure \
        will return an error response. The format of the responses are defined below",
    "get__firstline": "This endpoint will return capacity status information for the service specified for an authenticated and \
        authorised user.",
...
description_post = (
    ...
    + "Values specified in the request payload will be validated by the API. The following business validation rules are in \
    place: </BR><ul>"
    + "<li>"
    ...
    + "</li></ul> </BR> <B><U>Response Formats</B></U></BR> </BR> Please refer to the Response section of this document for the \
        response formats."
)
description_get = (
...
<pre>{ </BR>\
    "capacityStatus": [</BR>\
        "VAL-0002 - The given CapacityStatus value is invalid and must be a value as defined by the CapacityStatusRequestPayload \
            model." </BR>\
    ],</BR>\
    "resetStatusIn": [</BR>\
        "VAL-0004 - ResetStatusIn outside of limits - the reset time given is outside the minimum limit (0 minutes) defined by the \
            CapacityStatusRequestPayload model."</BR>\
    ]</BR>\
}</pre>'

authentication_error_response = 'Unauthorized - when a user is either no longer active in DoS or is not authenticated to use this \
    API. \
