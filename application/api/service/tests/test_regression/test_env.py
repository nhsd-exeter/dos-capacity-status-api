...
class TestEnv:
    ...
    inactive_key = None
    try:
        user = User.objects.db_manager("default").filter(username__contains="TestUser").delete()
    except ObjectDoesNotExist:
        pass
    finally:

        user = User.objects.db_manager("default").create_user(username="TestUser", password="fakePassword1")
    ...
    try:
        ...
        inactive_user = User.objects.db_manager("default").filter(username__contains="InActiveTestUser").delete()
    except ObjectDoesNotExist:
        pass
    finally:
    ...
    expected_reset_time_upper = current_time + timedelta(minutes=240) + timedelta(minutes=10)
    expected_reset_time_str_upper = expected_reset_time_upper.astimezone().strftime("%Y-%m-%dT%H:%M:%SZ")
    expected_reset_time_lower = current_time + timedelta(minutes=240) - timedelta(minutes=10)
    ...
    expected_modified_time_str_upper = expected_modified_time_upper.astimezone().strftime("%Y-%m-%dT%H:%M:%SZ")
    expected_modified_time_lower = current_time
    expected_modified_time_str_lower = expected_modified_time_lower.astimezone().strftime("%Y-%m-%dT%H:%M:%SZ")
