...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            api_user = User.objects.db_manager("default").filter(username__contains=api_username).delete()
        except ObjectDoesNotExist:
            pass
        finally:
            api_user = User.objects.db_manager("default").create_user(username=api_username, password=api_password)
